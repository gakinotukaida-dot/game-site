...
3. **点灯前のUI前提**: index.html / make_feed.py は現状 `layer` / `aligned` を見ずに全シグナルを同格の
   「きっかけ」として表示する。点灯すると降格済みの併発(context)シグナルまで主因のように表示されるため、
   点灯コミットと同時に (a) make_feed は context シグナルに「（併発）」等の弁別を付け、(b) index.html は
   `aligned=false` のチップを弱く表示（または `primary_cause` を優先）するよう更新すること。
4. しきい値確定後、本番 `view02_rank_v2.yml` に `TIMING_ALIGN: "1"` を追加して点灯（env のみ・可逆）。
...
### 既知の制限・負債（レビューで確認済み・意図的に残す）

- **トリガー判定の二重実装**: `_has_trigger_pre_web()`（GDELT優先度づけ用の事前判定）と行ループの
  シグナル構築が同じ条件の並行実装。新しいシグナル種を追加する際は**両方**を更新すること
  （片方だけだと GDELT 予算配分と公開シグナルが静かに食い違う）。恒久対処は「シグナル構築を
  Web照会の前に1回だけ行う」リファクタだが、テスト基盤が無い現状ではドリフト監視で足りると判断。
  なお investigation.results の hit は signals から導出済み＝この複製は2箇所のみ。
- **ゲーム名キーの衝突**: Twitch / GDELT の照会・結果（`tw` / `web` / `web_failed`）はゲーム名キー
  （外部APIが名前でしか引けないため・PR#39以前からの方式）。同名2作品が候補に並ぶと結果が混線する。
  発生はまれで影響は当該作品の調査メタに限られるため許容。厳密化するなら appid→照会名の射影を
  fetch 境界に置く。
