...
MAX_RUNTIME_SEC = int(os.environ.get("MAX_RUNTIME_SEC") or "1080")  # ハード上限。timeout に余裕を残す安全弁
# 長時間ループ（A'案）では、開始時に算出したターゲットを数時間使い続けると、途中で急上昇した
# 新顔/新作をそのジョブ中は見逃す。この値ごと（パス数）にターゲットを再算出して取り込む。
# 既定 0 ＝ 再算出しない（＝改修前と同一挙動・後方互換）。有効化は yml 側で明示する。
REFRESH_TARGETS_EVERY = int(os.environ.get("REFRESH_TARGETS_EVERY") or "0")
...
def main():
    ...
    print(f"観測パス: PASSES={PASSES}, INTERVAL_SEC={INTERVAL_SEC}, MAX_RUNTIME_SEC={MAX_RUNTIME_SEC}, "
          f"REFRESH_TARGETS_EVERY={REFRESH_TARGETS_EVERY}（timeout内に収める安全弁つき）")
    ...
    for i in range(PASSES):
        ...
        if i > 0:
            ...
            if wait > 0:
                ...
                time.sleep(wait)
            # 長時間ループ中、一定パスごとにターゲットを取り直して新顔/新作を取り込む（再算出は読み取りのみ）。
            if REFRESH_TARGETS_EVERY and i % REFRESH_TARGETS_EVERY == 0:
                try:
                    targets, n_risers, n_launched = get_targets()
                    print(f"  ターゲット再算出: {len(targets)} 件（昇格 {n_risers} / 発売直後 {n_launched}）")
                except Exception as e:
                    print(f"  ターゲット再算出に失敗（前回のターゲットを継続使用・非破壊）: {e}")
