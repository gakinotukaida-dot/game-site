...
def main():
    ...
    try:
        with conn.cursor() as cur:  # 読み取りのみ（commit しない）
            ...
            # ⑨-a：価格・割引・レビュー好評率（収集は既存のまま・ここは読み取りのみ）。
            # 取れなくても本体は止めない（fetch_market が内部で握って空を返す）。
            mkt = fetch_market(cur, [it.get("appid") for it in items])
            for it in items:
                it["_market"] = market_of(mkt, it.get("appid"))
    finally:
        conn.close()
